...
// overload operator, should conform SortedListClass
bool EventClass::operator <= (const EventClass &compareEvent) const
{
    bool isSmallerThan = false;
    
    if (this->timeOccurs <= compareEvent.getTimeOccurs())
    {
        isSmallerThan = true;
    }
